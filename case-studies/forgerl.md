...
The public application uses FastAPI, a sealed read-only SQLite archive and a dependency-free HTML/CSS/JavaScript interface on the portfolio VPS. The private research ledger uses SQLite/WAL under a separate runtime identity. Inference is a separate, explicitly budgeted hosted dependency. No continuously rented GPU is required to serve recorded experiments. The optional live trial uses a separate private Unix-socket broker with the model credential and original durable ledger. The public viewer has neither ledger, credential nor executor access. One fixed GPT-OSS20B request, one active job/two queued jobs, session/CSRF/Origin checks and $0.05 per UTC day/$1 per UTC month limits bound exposure; the original $8 public/$20 global lifetime caps remain unchanged. Live results never enter the research leaderboard. See the [deployed trial contract](https://github.com/stelioszach03/forgerl/blob/main/docs/RECRUITER_DEMO.md).
