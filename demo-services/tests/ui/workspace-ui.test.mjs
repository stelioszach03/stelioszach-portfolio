...
const result=(text,entities=[])=>{
 const chars=Array.from(text);let end=0;const parts=[];
 const rows=entities.map(e=>{const replacement=e.replacement??(e.action==='mask'?'*'.repeat(e.span[1]-e.span[0]):e.action==='hash'?e.label+'_HASH:synthetic-fixture':'[REDACTED:'+e.label+']');parts.push(chars.slice(end,e.span[0]).join(''),replacement);end=e.span[1];return{...e,replacement};});
 parts.push(chars.slice(end).join(''));return{original_len:chars.length,result_text:parts.join(''),entities:rows,time_ms:7};
};
...
test('DeID renders untrusted Unicode source/result/labels as text and exports only explicit metadata',async()=>{
 ...
 const ctx=await pageFor('/deid/',{deidHandler:async route=>{posted=route.request().postDataJSON();await route.fulfill({json:result(text,[{span:[start,points.length],label,detector:'regex',action:'redact',replacement:'<img src=x onerror="window.__xss=1">'}])});}});const{page,context,errors}=ctx;
 ...
 await page.locator('#entities .entity-open').click();assert.equal(await page.locator('#inspector .context strong').innerText(),'test@example.invalid');assert.equal(await page.locator('#inspector img').count(),0);
 assert.equal(await page.evaluate(()=>window.__copied.length),0);await page.locator('#exportMenu summary').click();await page.locator('#copySummary').click();const summary=JSON.parse(await page.evaluate(()=>window.__copied[0]));assert.equal(summary.review_required,true);assert.equal(summary.source_characters,points.length);assert.ok(!JSON.stringify(summary).includes('test@example.invalid'));assert.ok(!('result_text'in summary));assert.ok(summary.decisions.every(e=>!('surface'in e)&&!('text'in e)));
...
test('DeID filters detector/label, invalidates changed policy, and refuses stale in-flight results',async()=>{
 ...
 await page.locator('#text').fill(example.examples[0].text);await page.locator('#run').click();await waitReview(page);await page.locator('#detector').selectOption('regex');assert.equal(await page.locator('#entities tr').count(),1);assert.match(await page.locator('#entities').innerText(),/EMAIL/);await page.locator('#labelFilter').selectOption('PERSON');assert.equal(await page.locator('#entities tr').count(),0);assert.match(await page.locator('#noEntities').innerText(),/No detections match/);
