...
test('SMT suite makes every real request sequentially, matches actual expected codes and can inspect a response without changing draft',async()=>{
 const data=fixture('smt-verify');let active=0,maxActive=0;
 const ui=await setup('smt-verify',{post:async(request,n)=>{active++;maxActive=Math.max(maxActive,active);assert.deepEqual(request,data.responses[n-1].request);await tick();active--;return clone(data.responses[n-1].response);}});
 try{const draft=ui.$('candidate').value;ui.$('suite-run').click();await until(()=>ui.calls.length===data.responses.length&&!ui.$('suite-run').disabled);assert.equal(maxActive,1);assert.match(ui.$('suite-summary').textContent,/5\/5 responses · 5 matched/);assert.equal(ui.d.querySelectorAll('[data-state="matched"]').length,5);assert.equal(ui.$('candidate').value,draft);assert.equal(ui.$('workspace').dataset.mobilePanel,'result');ui.d.querySelector('[data-suite-inspect="1"]').click();assert.match(ui.$('out').textContent,/FAILED_CERTIFICATION/);assert.equal(ui.$('candidate').value,draft);assert.equal(ui.$('copy').disabled,false);ui.$('tab-input').click();assert.equal(ui.$('workspace').dataset.mobilePanel,'input');assert.equal(ui.errors.length,0);}finally{ui.close();}
});
test('SMT suite stop waits for the in-flight response and does not send or invent remaining cases',async()=>{
 const data=fixture('smt-verify');let finish;const ui=await setup('smt-verify',{post:()=>new Promise(r=>finish=r)});
 try{ui.$('suite-run').click();await until(()=>ui.calls.length===1);ui.$('suite-stop').click();assert.match(ui.$('status').textContent,/in-flight solver is not cancelled/);finish(clone(data.responses[0].response));await until(()=>!ui.$('suite-run').disabled);assert.equal(ui.calls.length,1);assert.equal(ui.d.querySelectorAll('[data-state="matched"]').length,1);assert.equal(ui.d.querySelectorAll('[data-state="skipped"]').length,4);}finally{ui.close();}
});
test('SMT suite transport failure records an error and stops without fake completion or retries',async()=>{
 const ui=await setup('smt-verify',{post:()=>({httpStatus:503,body:{detail:'busy'}})});try{ui.$('suite-run').click();await until(()=>ui.calls.length===1&&!ui.$('suite-run').disabled);assert.match(ui.$('suite-summary').textContent,/0\/5 responses · 0 matched/);assert.equal(ui.d.querySelectorAll('[data-state="error"]').length,1);assert.equal(ui.d.querySelectorAll('[data-state="skipped"]').length,4);assert.equal(ui.$('history').disabled,true);}finally{ui.close();}
});
test('SMT JSON diagnostics catch duplicate variables and malformed terms before submitting',async()=>{
 const ui=await setup('smt-verify');try{const p=clone(ui.data.responses[0].request);p.variables.push(clone(p.variables[0]));input(ui,'problem',JSON.stringify(p));ui.$('go').click();assert.match(ui.$('error').textContent,/Duplicate variable/);assert.equal(ui.calls.length,0);p.variables.pop();p.constraints[0].terms=[7];input(ui,'problem',JSON.stringify(p));ui.$('go').click();assert.match(ui.$('error').textContent,/terms must be an array of objects/);assert.equal(ui.calls.length,0);}finally{ui.close();}
});
test('Fraud network draws only actual bounded graph edges; account inspection and evidence tabs are functional',async()=>{
 const data=fixture('fraud-graph');const ui=await setup('fraud-graph');try{await run(ui);const n=data.responses[0].response.neighborhood;assert.ok(n?.nodes?.length);assert.equal(ui.$('out').querySelectorAll('.graph-edge').length,n.edges.length);assert.equal(ui.$('out').querySelectorAll('.graph-node').length,n.nodes.length);assert.equal(ui.$('graph-node').options.length,n.nodes.length);assert.equal(ui.$('panel-network').hidden,false);assert.equal(ui.$('panel-evidence').hidden,true);ui.$('graph-node').value='2';ui.$('graph-node').dispatchEvent(new ui.w.Event('change'));assert.match(ui.$('node-detail').textContent,new RegExp(`in-degree ${n.nodes[2].in_degree}`));assert.equal(ui.$('out').querySelector('[data-node-index="2"]').classList.contains('selected'),true);ui.$('out').querySelector('[data-inspector-tab="evidence"]').click();assert.equal(ui.$('panel-evidence').hidden,false);assert.equal(ui.$('panel-network').hidden,true);assert.match(ui.$('panel-evidence').textContent,/not comparable units or model importance/);}finally{ui.close();}
});
...
for(const name of ['smt-verify','fraud-graph'])test(`${name}: workspace tabs support arrow-key navigation and a roving tab stop`,async()=>{const ui=await setup(name);try{ui.$('tab-input').focus();ui.$('tab-input').dispatchEvent(new ui.w.KeyboardEvent('keydown',{key:'ArrowRight',bubbles:true}));assert.equal(ui.$('workspace').dataset.mobilePanel,'result');assert.equal(ui.$('tab-result').tabIndex,0);assert.equal(ui.$('tab-input').tabIndex,-1);ui.$('tab-result').dispatchEvent(new ui.w.KeyboardEvent('keydown',{key:'Home',bubbles:true}));assert.equal(ui.$('workspace').dataset.mobilePanel,'input');}finally{ui.close();}});
